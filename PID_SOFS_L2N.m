...
%	bound on Hinf norm by L2 function norms.
%
...
switch law
    case 'P'
LMISYS=LMI_P(Poly,gL2,sK_,Tk,KPlb,KPub);	
    
	case 'PI'
LMISYS=LMI_PI(Poly,gL2,sK_,Tk,KPlb,KPub,TIlb,TIub);

    case 'PID'
LMISYS=LMI_PID(Poly,gL2,sK_,Tk,PhiD,KPlb,KPub,TIlb,TIub,TDlb,TDub);
end 
...
function LMISYS=LMI_P(Poly,gL2,sK_,Tk,KPlb,KPub)
%Parameters
%Uk=abs(Tk); 	%Diagonalization matrix
Nx=Poly.Nx; 	%State variables size
Ny=Poly.Ny;		%Output variables size
...
function LMISYS=LMI_PI(Poly,gL2,sK_,Tk,KPlb,KPub,TIlb,TIub)
%Parameters
Uk=abs(Tk); 	%Diagonalization matrix
Nx=Poly.Nx; 	%State variables size
Ny=Poly.Ny;		%Output variables size
...
%H{i} < 0
	...
	lmiterm([neq 3 2 0],Bw{i}'*Bu{i}*Tk);
...
function LMISYS=...
	LMI_PID(Poly,gL2,sK_,Tk,PhiD,KPlb,KPub,TIlb,TIub,TDlb,TDub)
%Parameters
Uk=abs(Tk); 	%Diagonalization matrix
Nx=Poly.Nx; 	%State variables size
Ny=Poly.Ny;		%Output variables size
...
%H{i} < 0
	...
	lmiterm([neq 4 2 0],Bw{i}'*Bu{i}*Tk);
	lmiterm([neq 4 3 0],Bw{i}'*Bu{i}*Tk);
