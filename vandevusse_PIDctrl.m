function [sys,x0,str,ts,simStateCompliance]=vandevusse_PIDctrl(t,x,u,flag,x0,par)
%vandevusse - Van de Vusse non-isothermal CSTR non-linear model 
%S-Function under PID control
%
...
%
%[sys,x0,str,ts,simStateCompliance]=vandevusse_PIDctrl(t,x,u,flag,x0,par)
%
...
%TD1:		loop #1 derivative time 					[min]
%TD2:		loop #2 derivative time 					[min]
%PhiD1:		loop #1 der. action low-pass filter const. 	[1/min]
%PhiD2:		loop #2 der. action low-pass filter const. 	[1/min]
...
%x(7): uD1  loop #1 derivative action					[]
%x(8): uD2  loop #2 derivative action					[]
%--------------------------------------------------------------------
%----- Inputs -------------------------------------------------------
...
%----- S-FUNCTION BUILDING PARAMETERS AND INITIAL CONDITIONS --------
    case 0,        
sys(1)=8;	%Number of continuous states
...
th = th0 + KP1*(dInt1dt + Int1/TI1 + TD1*uD1);

%u(2): Qc	cooling medium heat removal rate			[kJ/min]
Qc = Qc0 + KP2*(dInt2dt + Int2/TI2 + TD2*uD2);
...
duD1dt = -PhiD1*uD1 - PhiD1*dCBdt;

duD2dt = -PhiD2*uD2 - PhiD2*dTdt;

%dxdt allocation:
dxdt=[dCAdt; dCBdt; dTdt; dTcdt; dInt1dt; dInt2dt; duD1dt; duD2dt];
